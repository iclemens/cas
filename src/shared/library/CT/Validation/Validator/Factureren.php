<?php
	/**
	 * CT_Validator_Validator_Factureren, Citrus-IT Online Boekhouding
	 *
	 ...
	 * @package boekhouding
	 */
	class CT_Validation_Validator_Factureren extends CT_Validation_Validator
	{
		...
		public function validate(&$object, &$error)
		{
			...
			if(array_key_exists('regels', $object)) {
				foreach($object['regels'] as $nr => $regel) {
					...
					if(strlen($regel['omschrijving']) > 75) {
						$error->reject('Omschrijving mag maximaal 75 tekens bevatten');
					}
